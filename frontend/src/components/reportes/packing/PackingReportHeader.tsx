...
interface PackingReportHeaderProps {
  onExportPDF: () => void;
  onExportExcel: () => void;
  reportId: string;
  generatedAt: string;
...
export default function PackingReportHeader({
  onExportPDF,
  onExportExcel,
  reportId,
  generatedAt,
  ...
  return (
    <>
      ...
      >
        ...
        >
          LISTADO Y RESUMEN DE PACKINGS RECIBIDOS
        </div>
        <div
          className="text-[10px] mt-1"
          style={{ color: 'var(--report-text-muted)' }}
        >
          Filtro aplicado: {clientName} | Período: {formatDate(dateFrom)} al {formatDate(dateTo)}
        </div>
      </div>
    </>
  );
