...
interface ProcesosReportHeaderProps {
  onExportPDF: () => void;
  onExportExcel: () => void;
  reportId: string;
  generatedAt: string;
...
export default function ProcesosReportHeader({
  onExportPDF,
  onExportExcel,
  reportId,
  generatedAt,
    ...
    <>
      ...
      >
        ...
        <div
          ...
          style={{ color: 'var(--report-text-muted)' }}
        >
          Filtro aplicado: {proveedorName} | Período: {formatDate(dateFrom)} al {formatDate(dateTo)}
        </div>
      </div>
    </>
  );
